...
def run_item_sync_from_hook(doc, method):
    ...
    if (
        ...
        frappe.msgprint(
            _("Background sync to WooCommerce triggered for {}").format(
...
class SynchroniseItem(SynchroniseWooCommerce):
    ...
    def get_erpnext_item(self):
        ...
        if not all(
            ...
            raise ValueError(
                "Both woocommerce_server and woocommerce_id required")
                ...
                if image_url:
                    ...
                    if erp_image_date:
                        ...
                        current_images = json.loads(
                            wc_product.images) if wc_product.images else []

                        if current_images:
                            wc_image_date_str = current_images[0].get(
                                "date_modified")
                            try:
                                wc_image_date = datetime.fromisoformat(
                                    wc_image_date_str) if wc_image_date_str else None
                            except ValueError:
                                wc_image_date = None
                                frappe.log_error(
                                    f"Invalid date format for WooCommerce image: {wc_image_date_str}")

                            if not wc_image_date or erp_image_date > wc_image_date:  # Compare dates
                                new_image = {"id": current_images[0].get(
                                    "id"), "src": image_url, "date_created": date_created_str}
    ...
    def create_woocommerce_product(self, item: ERPNextItemToSync) -> None:
        ...
        # Image Synchronization
            if item.item.image:
                ...
                image_details = frappe.db.get_value("File", {"file_url": item.item.image}, [
                    "file_name", "file_url", "is_private", "content_hash", "creation"])
                if image_details:
                    image_url = format_erpnext_img_url(image_details)
                    if image_url:
                        date_created_str = image_details[4].isoformat(
                        ) if image_details[4] else datetime.now().isoformat()  # Convert to ISO format
                        wc_product.images = json.dumps(
                            [{"src": image_url, "date_created": date_created_str}])  # modified date
        ...
        if wc_product.attributes:
            ...
            for wc_attribute in wc_attributes:
                if frappe.db.exists("Item Attribute", wc_attribute["name"]):
                    ...
                    item_attribute = frappe.get_doc(
                        "Item Attribute", wc_attribute["name"])
                else:
                ...
                options = (
                    wc_attribute["options"] if wc_product.type == "variable" else [
                        wc_attribute["option"]]
                )
            ...
            if wc_server.item_field_map:
                for map in wc_server.item_field_map:
                    erpnext_item_field_name = map.erpnext_field_name.split(
                        " | ")
                    woocommerce_product_field_value = self.woocommerce_product.get(
...
def get_list_of_wc_products(
    ...
    if date_time_from:
        filters.append(
            ["WooCommerce Product", "date_modified", ">", date_time_from])
    if item:
        filters.append(["WooCommerce Product", "id", "=",
...
def get_item_price_rate(item: ERPNextItemToSync):
    ...
    if wc_server.enable_price_list_sync:
        item_prices = frappe.get_all(
            "Item Price",
            filters={"item_code": item.item.item_name,
...
def clear_sync_hash_and_run_item_sync(item_code: str):
    ...
    iwss = (
        frappe.qb.from_(iws).where(iws.enabled == 1).where(
            iws.parent == item_code).select(iws.name)
    ).run(as_dict=True)
